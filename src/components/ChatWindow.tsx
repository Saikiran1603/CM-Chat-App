...
import { messages as allMessages, getContact, currentUser } from '@/data/mockData';
...
export default function ChatWindow({ chat }: { chat: Chat }) {
  ...
  const { chats, toggleMute, togglePin, deleteChat, blockContact, unblockContact, isBlocked } = useAppData();
  ...
  const [msgs, setMsgs] = useState<Message[]>(() => allMessages.filter((m) => m.chatId === chat.id));
  ...
  // Reset the visible thread whenever the selected chat changes.
  useEffect(() => {
    setMsgs(allMessages.filter((m) => m.chatId === chat.id));
    setPanelTab(null);
  }, [chat.id]);
  ...
  function toggleStar(id: string) {
    setMsgs((prev) => prev.map((m) => (m.id === id ? { ...m, starred: !m.starred } : m)));
  }
    ...
    <div className="relative flex h-full min-w-0 flex-1">
      <div className="flex h-full min-w-0 flex-1 flex-col" style={{ backgroundColor: wallpaper }}>
        ...
        <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto px-3 py-4 sm:px-8">
          {msgs.length === 0 && <p className="mt-10 text-center text-sm text-muted">No messages yet. Say hello 👋</p>}
          {msgs.map((m) => (
...
function MessageBubble({ message: m }: { message: Message }) {
  ...
  const incomingCls = 'rounded-bl-sm bg-bubbleIn text-slate-800 dark:bg-white/10 dark:text-slate-100';
  ...
  if (m.type === 'doc') {
    return (
      ...
      >
        <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${m.outgoing ? 'bg-white/20' : 'bg-white dark:bg-white/10'}`}>

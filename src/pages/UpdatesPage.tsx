...
import { updates as fallbackUpdates, currentUser } from '@/data/mockData';
import { useContacts } from '@/context/ContactsContext';
import { useApiResource } from '@/hooks/useApiResource';
import { api } from '@/api/client';
import { Update, Contact } from '@/types';
import Avatar from '@/components/Avatar';
import EmptyState from '@/components/EmptyState';

export default function UpdatesPage() {
  const { getContact } = useContacts();
  const { data: updates, setData: setUpdates } = useApiResource<Update>('updates', fallbackUpdates);
  ...
  function open(id: string) {
    ...
    api.update('updates', id, { viewed: true }).catch(() => {});
  }
    ...
    <div className="flex h-full w-full">
      <div className={`flex h-full w-full flex-col border-r border-border bg-panel dark:border-white/10 dark:bg-[#15161f] md:w-[340px] ${active ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-none px-1">
          ...
          {unviewed.map((u) => (
            <UpdateRow key={u.id} update={u} contact={getContact(u.contactId)} onClick={() => open(u.id)} />
          ))}
          {viewed.length > 0 && (
      ...
      <div className={`h-full min-w-0 flex-1 bg-[#0b0c12] ${active ? 'flex' : 'hidden md:flex'}`}>
        {active ? (
          <UpdateViewer update={active} contact={getContact(active.contactId)} onClose={() => setActiveId(null)} onPrev={() => step(-1)} onNext={() => step(1)} />
        ) : (
...
function UpdateRow({ update, contact, onClick }: { update: Update; contact?: Contact; onClick: () => void }) {
...
function UpdateViewer({ update, contact, onClose, onPrev, onNext }: { update: Update; contact?: Contact; onClose: () => void; onPrev: () => void; onNext: () => void }) {
  if (!contact) return null;
  return (

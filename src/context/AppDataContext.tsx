...
export function AppDataProvider({ children }: { children: ReactNode }) {
  ...
  // Blocked contacts are their own json-server resource ("blocked"); load
  // them separately since they don't have bundled fallback data.
  useEffect(() => {
    api
      .getAll<BlockedRecord>('blocked')
      .then(setBlockedRecords)
      .catch(() => setBlockedRecords([]));
  }, []);

  const blockedIds = blockedRecords.map((b) => b.contactId);
  ...
  function blockContact(contactId: string) {
    if (blockedIds.includes(contactId)) return;
    const record: BlockedRecord = { id: `blk-${contactId}`, contactId };
    setBlockedRecords((prev) => [...prev, record]);
    api.create('blocked', record).catch(() => {});
  }

  function unblockContact(contactId: string) {
    setBlockedRecords((prev) => prev.filter((b) => b.contactId !== contactId));
    api.remove('blocked', `blk-${contactId}`).catch(() => {});
  }
